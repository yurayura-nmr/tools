...
Read APBS (version 1 or 3) potential (kT/e) files and compute difference map.
Manually remove header and footer lines and keep only numbers for processing.

Example: ./deltaEpot pH5.dx pH6.dx
i.e., if using: g++ deltaEpot.cpp -o deltaEpot
...
int process (char *filename_1, char *filename_2) {
    // Open the input and output files. [first pH, second pH] 
    ...
    // Read APBS data for both files for each point. Each contains three numbers.
    double in1_a1, in1_a2, in1_a3;
    double in2_a1, in2_a2, in2_a3;
   
    int timeStep; // Count the number of lines containing data 
    timeStep = 0;
